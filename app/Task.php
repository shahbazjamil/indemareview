...
class Task extends BaseModel
{
     ...
     * @param null $userID
     */
    public static function projectOpenTasks($projectId, $userID = null, $startDate = null)
    {
        $taskBoardColumn = TaskboardColumn::where('slug', 'completed')->first();
        if(!is_null($startDate)) {
            $projectTask = Task::join('task_users', 'task_users.task_id', '=', 'tasks.id')->where('tasks.due_date', '>=', $startDate->format('Y-m-d'))->where('tasks.board_column_id', '<>', $taskBoardColumn->id)->select('tasks.*');
        } else {
        ...
        $projectTask->where('project_id', $projectId);
         if(!is_null($startDate)) {
             // to avoid duplicate rows on project dashboard
             $projectTask->groupBy('task_users.user_id');
         }
         
         $projectTask->orderBy('id', 'desc');
