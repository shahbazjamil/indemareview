...
class VendorsDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn(
                'name',
                function ($row) {
                return ucwords($row->vendor_name);
            ...
            ->addColumn(
                'vendor_rep_name',
                function ($row) {
                    return ucwords($row->vendor_rep_name);
                }
            )
            ->addColumn(
                'rep_phone',
                function ($row) {
                    return $row->rep_phone;
                }
            )
            ...
            ->editColumn(
                'company_name',
                function ($row) {
                    return '<a href="javascript:;" class="vendor-detail" data-vendor-id="' . $row->id . '">' . ucwords($row->company_name) . '</a>';
                }
            )
            ...
            ->addIndexColumn()
            ->rawColumns(['name', 'action', 'status','credentials', 'company_name']);
    }
    ...
    {
        $model =  $model->select('id', 'vendor_name', 'company_name', 'vendor_email', 'vendor_category', 'vendor_markup',  'status', 'created_at', 'url', 'user','password', 'vendor_rep_name' ,'tags' ,'rep_phone');
    ...
    protected function getColumns()
    {
        return [
            ...
            //'Rep Name' => ['data' => 'vendor_rep_name', 'name' => 'vendor_rep_name'],
            __('app.email') => ['data' => 'vendor_email', 'name' => 'vendor_email'],
            'Phone Number' => ['data' => 'rep_phone', 'name' => 'rep_phone'],
