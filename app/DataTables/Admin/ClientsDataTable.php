...
//<li><a href="javascript:;" data-user-id="' . $row->user_id . '" class="login-view"><i class="fa fa-search" aria-hidden="true"></i> Login </a></li>
                //<li><a href="javascript:;" data-user-id="' . $row->user_id . '" class="login-view"><i class="fa fa-search" aria-hidden="true"></i> Login </a></li>
                $action .= '</ul> </div>';
            ...
            ->editColumn(
                ...
                function ($row) {
                    ...
                    return  '<div class="row truncate"><div class="col-sm-3 col-xs-4">' . $image . '</div><div class="col-sm-9 col-xs-8"><a href="' . route('admin.clients.show', $row->user_id) . '">' . ucwords($row->name) . '</a></div></div>';
    ...
    public function query(ClientDetails $model)
    {
        ...
        $model = $model->join('users', 'client_details.user_id', '=', 'users.id')
            ->select('client_details.id', 'client_details.user_id', 'client_details.name', 'client_details.company_name', 'client_details.email', 'client_details.created_at', 'users.status', 'client_details.tags')
