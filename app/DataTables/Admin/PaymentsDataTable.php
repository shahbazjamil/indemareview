...
//            Aqeel Code for Vendor ID
            ->editColumn('vendorName', function ($row) {
                //echo $row;
                //return $row->vendor->name;
                if (isset($row->vendor) && !is_null($row->vendor->name)) {
                    return $row->vendor->name;
                } else {
                    return '';
                }
...
//                return $symbol . number_format((float) $row->amount, 2, '.', '') . ' (' . $code . ')';
    ...
    public function query(Payment $model)
    {
        ...
        $model = $model->with(['project:id,project_name','vendor', 'currency:id,currency_symbol,currency_code', 'invoice'])
            ->leftJoin('projects', 'projects.id', '=', 'payments.project_id')
            //->leftJoin('vendors', 'vendors.id', '=', 'payments.vendor_id')
            ->select('payments.id', 'payments.vendor_id',  'payments.project_id', 'payments.currency_id', 'payments.invoice_id', 'payments.amount', 'payments.status', 'payments.paid_on', 'payments.remarks', 'payments.bill', 'payments.payment_type');
    ...
    protected function getColumns()
    {
        ...
        return [
            ...
            __('app.vendor') . '#' => ['data' => 'vendorName'],
            __('modules.invoices.amount') => ['data' => 'amount', 'name' => 'amount'],
            'Type' => ['data' => 'payment_type', 'name' => 'payment_type'],
            'Paid On / Refund On' => ['data' => 'paid_on', 'name' => 'paid_on'],
