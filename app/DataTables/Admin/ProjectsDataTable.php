    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('members', function ($row) {
                ...
                if (count($row->members) > 0) {
                    foreach ($row->members as $member) {
                        if(isset($member->user) && isset($member->user->image)){
                            $members .= '<img data-toggle="tooltip" data-original-title="' . ucwords($member->user->name) . '" src="' . $member->user->image_url . '"
                            alt="user" class="img-circle" width="25" height="25"> ';
                        } else {
                            $members .= '<span data-toggle="tooltip" data-original-title="' . ucwords($member->user->name) . '" data-toggle="tooltip" data-original-title="' . ucwords($member->user->name) . '" class="nameletter">'.company_initials().'</span>';
                        }
            ...
            ->addColumn('client_id', function ($row) {
                ...
                $clients = ucwords($row->client->name);
                
                foreach ($row->clients as $client) {
                    if($client->client && $client->client->id != $row->client_id) {
                        $clients .=' ,'.ucwords($client->client->name);
                    }
                }
                return $clients;
