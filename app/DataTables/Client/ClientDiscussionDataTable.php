...
class ClientDiscussionDataTable extends BaseDataTable
{
    /**
            ...
            ->editColumn('title', function ($row) {
                ...
                if($row->user->image) {
                    $title.= '<img src="'.$row->user->image_url.'" class="img-circle" width="35" height="35" />';
                     ...
                     $title.= '<span class="nameletter">'.company_initials().'</span>';
                }
                ...
                if (!is_null($row->project_id)) {
                    $title.= '<a href="'.route('client.projects.discussionReplies', [$row->project_id, $row->id]).'" class="text-dark">' . ucwords($row->title) . '</a>';
                } else {
                    $title.= '<a href="'.route('client.discussion.show', [$row->id]).'" class="text-dark">' . ucwords($row->title) . '</a>';
                }
