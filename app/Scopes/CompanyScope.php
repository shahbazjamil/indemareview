...
//        echo "this is ".session('user');
        ...
        if (!session()->has('client_company')) {
            ...
            //echo Schema::hasColumn($model->getTable(), 'company_id');
            if (auth()->hasUser() && Schema::hasColumn($model->getTable(), 'company_id')) {
                $company = company();

                if ($company) {

                    $builder->where($model->getTable() . '.company_id', '=', $company->id)->orWhereNull($model->getTable() . '.company_id');
                }

//                else{
//                    $builder =  $model->getTable();
//                }
