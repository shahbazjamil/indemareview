...
use App\DataTables\Member\MemberPurchaseOrdersDataTable;
use App\Estimate;
...
class MemberPurchaseOrdersController extends MemberBaseController
{
    //bitsclan code here
    protected $setting = '';
    ...
    public function index(MemberPurchaseOrdersDataTable $dataTable)
    {
        $this->mixPanelTrackEvent('view_page', array('page_path' => '/member/purchase-orders'));
        $this->pageTitle = 'Purchase Orders';
        $this->projects = Project::all();
        $this->clients = User::allClients();
        $this->totalRecords = PurchaseOrder::count();
        
        return $dataTable->render('member.purchase-orders.index', $this->data);
    }
...
//    }
    ...
    public function create(Request $request)
    {
        ...
        return view('member.purchase-orders.create', $this->data);
    ...
    public function store(StorePurchaseOrder $request)
    {
        ...
        //log search
        $this->logSearchEntry($po->id, 'PurchaseOrder ' . $po->purchase_order_number, 'member.purchase-orders.show', 'purchase_order');

        return Reply::redirect(route('member.purchase-orders.index'), 'Purchase Order Created');
    }
    ...
    public function edit($id)
    {
        ...
        return view('member.purchase-orders.edit', $this->data);
    }
    ...
    public function update(UpdatePurchaseOrder $request, $id)
    {
        ...
        return Reply::redirect(route('member.purchase-orders.index'), 'Purchase Order Updated');
    }
...
//        $this->invoiceSetting = InvoiceSetting::first();
//        return view('member.purchase-orders.show', $this->data);
//    }
...
//        }
        ...
        $this->taxes = Tax::all();
        $view = view('member.purchase-orders.add-item', $this->data)->render();
    ...
    public function checkShippingAddress()
    {
        if (request()->has('clientId')) {
            ...
            if (request()->showShipping == 'yes' && (is_null($user->client_details->shipping_address) || $user->client_details->shipping_address === '')) {
                $view = view('member.invoices.show_shipping_address_input')->render();
    ...
    public function domPdfObjectForDownload($id)
    {
        ...
        $pdf->loadView('member.purchase-orders.po-pdf', $this->data);
