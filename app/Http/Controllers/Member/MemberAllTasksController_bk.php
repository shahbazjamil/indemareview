    ...
    public function data(Request $request, $hideCompleted = null, $projectId = null)
    {
        ...
        return DataTables::of($tasks)
            ...
            ->editColumn('created_by', function ($row) {
                ...
                if (!is_null($row->created_by)) {
                    return ($row->created_image) ? '<img src="' . asset_url('avatar/' . $row->created_image) . '"
                                                            alt="user" class="img-circle" width="30" height="30"> ' . ucwords($row->created_by) : '' .$nameletter . '"
            ...
            ->editColumn('heading', function ($row) {
                ...
                return '<a href="javascript:;" data-task-id="' . $row->id . '" class="show-task-detail">' . ucfirst($row->heading) . '</a>'.$pin;
            })
...
//            }
            ...
            $view = '';
            return Reply::successWithData(__('messages.taskUpdatedSuccessfully'), ['html' => $view, 'tagsArr' => $tagsArr, 'description' => $task->description , 'assigned_users' => $assigned_users]);
        }
    }
