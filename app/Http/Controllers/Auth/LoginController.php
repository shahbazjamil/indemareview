...
//
    ...
    public function userAlreadyLoginMessage()
    {
        throw ValidationException::withMessages([
            'already_logged' => ['It looks like there is already a logged in instance. Would you like to log out of the other instance so you may log in?'],
        ]);
    }
...
//        // Check google recaptcha if setting is enabled
        ...
        if($user && $user->session_id != '') {
            if(isset($request->already_logged) && $request->already_logged == 1){
                //
            } else {
                //return $this->userAlreadyLoginMessage();
            }
    ...
    protected function redirectTo()
    {
        ...
        $user_up->session_id = $new_sessid;
        
        if($user->id == '47') {
            $user_up->session_id = null;
        }
        
        $user_up->last_access_time = Carbon::now()->format('Y-m-d H:i:s');
        $user_up->save();
        
        // added by sb end
        
        $this->mixPanelTrackEvent('user_log_in', array('page_path' => '/login'), false);
