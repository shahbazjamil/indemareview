...
class AdminContractController extends AdminBaseController
{
    ...
    public function index(ContractsDataTable $dataTable) {
        ...
        $this->mixPanelTrackEvent('view_page', array('page_path' => '/admin/contracts'));
...
//            ->count();
        ...
        $this->expiredCounts = 0;
        $this->aboutToExpireCounts = 0;
        
        $this->totalRecords = $this->contractCounts;
    ...
    public function create() {
        ...
        $this->contractTemplates = ContractTemplate::orderBy('template_name')->get();
    ...
    public function store(StoreRequest $request) {
        ...
        $this->mixPanelTrackEvent('contract_created', array('page_path' => '/admin/contract/create'));
    ...
    public function show($id) {
        ...
        if(!empty($this->contract->contract_detail)) {
            $this->contract->contract_detail = contract_variable_replacer($this->contract->id);
        }
        
        //echo $this->contract->contract_detail;exit;
    ...
    private function storeUpdate($request, $contract)
    {
        ...
        $contract->amount = $request->amount ? $request->amount : 0;
        $contract->original_amount = $request->amount ? $request->amount : 0;
...
//        $contract->original_end_date = Carbon::parse($request->end_date)->format('Y-m-d');
        ...
        if(isset($request->template_id) && !is_null($request->template_id)) {
            $contract->template_id = $request->template_id;
            $template = ContractTemplate::findOrFail($request->template_id);
            
            if($template) {
               $contract->contract_detail = $template->template_summary;
            }
        }
