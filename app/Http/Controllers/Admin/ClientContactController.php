...
class ClientContactController extends AdminBaseController
{
    ...
    public function store(StoreContact $request)
    {
        ...
        // assign client projects to contacts
        ContactProject::where('contact_id', $contact->id)->delete();
        $projects = Project::clientProjects($request->user_id);
        ...
        if(count($project_ids) > 0) {
            foreach ($project_ids as $project_id) {
                ...
                $contactProject->contact_id = $contact->id;
    ...
    public function update(StoreContact $request, $id)
    {
       ...
       // assign client projects to contacts
        ContactProject::where('contact_id', $contact->id)->delete();
        $projects = Project::clientProjects($contact->user_id);
