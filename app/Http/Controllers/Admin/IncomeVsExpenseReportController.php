...
class IncomeVsExpenseReportController extends AdminBaseController
{
    ...
    {
        $this->mixPanelTrackEvent('view_page', array('page_path' => '/admin/reports/income-expense-report'));
        ...
        $this->totalIncomes = $this->getTotalIncome($this->fromDate->format('Y-m-d'), $this->toDate->format('Y-m-d') , $this->projectID);
        $this->totalExpenses = $this->getTotalExpense($this->fromDate->format('Y-m-d'), $this->toDate->format('Y-m-d') , $this->projectID);
    ...
    public function store(Request $request)
    {
        ...
        $this->projectID = $request->projectID ? $request->projectID : 'all';

        $this->totalIncomes = $this->getTotalIncome($this->fromDate, $this->toDate, $this->projectID);
        $this->totalExpenses = $this->getTotalExpense($this->fromDate, $this->toDate, $this->projectID);
        $this->graphData = $this->getGraphData($this->fromDate, $this->toDate, $this->projectID);
        return Reply::successWithData(__('messages.reportGenerated'), $this->data);
    }

    public function getGraphData($fromDate, $toDate, $projectID = 'all')
    {
        $graphData = [];
        ...
        if($projectID != 'all') {
            ...
            ->where('payments.project_id', $projectID)
            // ->groupBy('year', 'month')
            ->orderBy('paid_on', 'ASC')
            ->get([
        ...
        foreach ($invoices as $invoice) {
            ...
            if ($invoice->currency_id != $this->global->currency->id  && $invoice->price > 0 && $invoice->currency->exchange_rate > 0) {
    ...
    public function getTotalIncome($fromDate, $toDate, $projectID = 'all')
    {
...
//                            ->groupBy('year', 'month')
        ...
        foreach($invoices as $invoice) {
            if($invoice->currency_id != $this->global->currency->id && $invoice->price > 0 && $invoice->currency->exchange_rate > 0){
                $totalIncome += floor($invoice->total / $invoice->currency->exchange_rate);
            }
            else{
    ...
    public function getTotalExpense($fromDate, $toDate , $projectID = 'all')
    {
        // $fromDate = Carbon::createFromFormat($this->global->date_format, $fromDate)->toDateString();
        // $toDate = Carbon::createFromFormat($this->global->date_format, $toDate)->toDateString();
        $totalExpense = 0;
