...
use App\DataTables\Admin\SuperAdminTicketDataTable;
...
class SuperAdminTicketsController extends SuperAdminBaseController
{
...
//        });
    ...
    public function index(SuperAdminTicketDataTable $dataTable)
    {
        ...
        return $dataTable->render('super-admin.tickets.index', $this->data);
    }
    ...
    public function getGraphData($fromDate, $toDate, $agentId = null, $status = null, $priority = null, $channelId = null, $typeId = null)
    {
        ...
        $total = [];
        $totalTickets = Ticket::withoutGlobalScope(CompanyScope::class)->where(DB::raw('DATE(`created_at`)'), '>=', $fromDate)
        ...
        $resolved = [];
        $resolvedTickets = Ticket::withoutGlobalScope(CompanyScope::class)->where(DB::raw('DATE(`updated_at`)'), '>=', $fromDate)
        ...
        $unresolved = [];
        $unresolvedTickets = Ticket::withoutGlobalScope(CompanyScope::class)->where(DB::raw('DATE(`updated_at`)'), '>=', $fromDate)
            ->where(DB::raw('DATE(`updated_at`)'), '<=', $toDate)
            ->where(function ($query) {
                $query->where('status', '<>', 'closed');
    ...
    public function create()
    {
        ...
        $this->lastTicket = Ticket::withoutGlobalScope(CompanyScope::class)->orderBy('id', 'desc')->first();
        $this->upload = can_upload();
        return view('super-admin.tickets.create', $this->data);
    }
    ...
    public function store(StoreTicket $request)
    {
        ...
        // $reply->user_id = '2'; //Reply From Super Admin
        ...
        //log search
        $this->logSearchEntry($ticket->id, 'Ticket: ' . $ticket->subject, 'super-admin.tickets.edit', 'ticket');

        return Reply::dataOnly(['ticketReplyID' => $reply->id]);

        return Reply::redirect(route('super-admin.tickets.index'), __('messages.ticketAddSuccess'));
    }

    public function edit($id)
    {
        $this->ticket = Ticket::withoutGlobalScope(CompanyScope::class)->findOrFail($id);
        $this->groups = TicketGroup::all();
        $this->types = TicketType::all();
        $this->channels = TicketChannel::all();
        $this->templates = TicketReplyTemplate::all();
...
//        echo $this->ticket;
        ...
        return view('super-admin.tickets.edit', $this->data);
    }

    public function update(UpdateTicket $request, $id)
    {
        DB::beginTransaction();
        $ticket = Ticket::withoutGlobalScope(CompanyScope::class)->findOrFail($id);
        ...
        // $ticket->channel_id = $request->channel_id;
        $ticket->save();
        DB::table('tickets')->where('id', $ticket->id)->update(array('company_id' =>$company_id));
        ...
        if ($request->message != '') {
            ...
            $lastMessage = view('super-admin.tickets.last-message', compact('reply', 'global'))->render();
    ...
    {
        Ticket::withoutGlobalScope(CompanyScope::class)->delete($id);
    ...
    {
        $openTickets = Ticket::withoutGlobalScope(CompanyScope::class)->select(DB::raw('count(`id`) as total'))
            ->where('status', 'open')
        ...
        $pendingTickets = Ticket::withoutGlobalScope(CompanyScope::class)->select(DB::raw('count(`id`) as total'))
        ...
        $resolvedTickets = Ticket::withoutGlobalScope(CompanyScope::class)->select(DB::raw('count(`id`) as total'))
        ...
        $closedTickets = Ticket::withoutGlobalScope(CompanyScope::class)->select(DB::raw('count(`id`) as total'))
            ->where('status', 'closed')
            ->where(DB::raw('DATE(`updated_at`)'), '>=', $startDate)
            ->where(DB::raw('DATE(`updated_at`)'), '<=', $endDate);
        ...
        $totalTickets = Ticket::withoutGlobalScope(CompanyScope::class)->select(DB::raw('count(`id`) as total'))
    ...
    public function export($startDate = null, $endDate = null, $agentId = null, $status = null, $priority = null, $channelId = null, $typeId = null)
    {
        ...
        $tickets = Ticket::withoutGlobalScope(CompanyScope::class)->join('users', 'users.id', 'tickets.user_id')
