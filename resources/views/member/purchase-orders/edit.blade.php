...
<div class="row bg-title">
    ...
    <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
        <ol class="breadcrumb">
            ...
            <li><a href="{{ route('member.purchase-orders.index') }}">{{ __($pageTitle) }}</a></li>
                                ...
                                <div class="form-group">
                                    ...
                                    <div>
                                        <div class="input-group">
                                            <input type="text" class="form-control readonly-background" readonly
                        ...
                        <div class="row">
											...
											<div class="row">
												<div class="col-md-12">
													<select onchange="getVendorDetail(this)" class="select2 form-control" data-placeholder="Choose Vendor"
														...
														<option
                                                                                                                    @if($po->vendor_id == $vendor->id) selected @endif 
                                                                                                                    value="{{ $vendor->id }}">{{ ucwords($vendor->vendor_name) }} - {{ ucwords($vendor->company_name) }}
														</option>
														@endforeach
													</select>
												</div>
									...
									<div class="col-md-6">
										<div class="form-group">
											...
											<div class="input-icon">
												<input type="text" class="form-control" name="contact" id="contact" value="{{ $po->contact }}">
											</div>
										</div>
									</div>	
									<div class="col-md-6">								
										<div class="form-group">
											...
											<div class="input-icon">
												<input type="text" class="form-control" name="company" id="company" value="{{ $po->company }}">
											</div>
										</div>
									</div>
								</div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <div class="input-icon">
                                        <textarea name="address" id="address" class="form-control">{{ $po->address }}</textarea>
							...
							<div class="col-md-3">			
										<div class="form-group">
											...
											<div class="input-icon">
												<input type="email" class="form-control" name="email" id="email" value="{{ $po->email }}">
											</div>
										</div>
										<div class="form-group">
											<label class="control-label">Account #</label>
											<div class="input-icon">
												<input type="text" class="form-control" name="account_no" id="account_no" value="{{ $po->account_no }}">
											</div>
										</div>
							</div>
                            <div class="col-md-4" style="display:none;">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="currency_id" id="currency_id">
                                        ...
                                        <option value="{{ $currency->id }}" @if($po->currency_id == $currency->id)
                                            selected
							...
							<div class="row">								
								<div class="col-md-4">
									<div class="form-group">
										...
										<div class="input-icon">
											<textarea name="shipping_address" id="shipping_address" class="form-control">{{ $po->shipping_address }}</textarea>
											...
											<div class="col-md-12">
												<div class="input-icon">
													<input type="text" class="form-control" name="purchase_order_date"
														...
														value="{{ $po->purchase_order_date->format($global->date_format) }}">
												</div>
											</div>
										</div>
									</div>
								...
								<div class="col-md-2">
									<div class="form-group">
										...
										<select class="form-control" name="terms" id="terms">
											<option @if($po->terms == 'due_on_receipt') selected @endif value="due_on_receipt" >Due on receipt</option>
											<option @if($po->terms == 'net_15') selected @endif value="net_15" >Net 15</option>
											<option @if($po->terms == 'net_30') selected @endif value="net_30" >Net 30</option>
											<option @if($po->terms == 'net_60') selected @endif value="net_60" >Net 60</option>
											<option @if($po->terms == 'custom') selected @endif value="custom" >Custom</option>
										</select>
									</div>
								</div>
                                                            <div class="col-md-2">
                                                                  <div class="form-group">
                                                                    ...
                                                                    <select name="project_id" id="project_id" class="form-control">
                                                                      <?php $default_project_id = $po->project_id ? $po->project_id : 0;?>
                                                            ...
                                                            <div class="form-group">
                                                                ...
                                                                <select name="status" id="status" class="form-control">
                                                                    @forelse($status as $sts)
                                                                        <option @if($po->status_id == $sts->id) selected
								...
								<div class="col-md-12 border-bottom">
									<div class="btn-group m-b-10">
										<button aria-expanded="false" data-toggle="dropdown"
											...
											type="button" onclick="$('#produbctsModal').show();">@lang('app.menu.products') <span class=""> + </span>
										</button>
								...
								<div class="col-xs-8 m-t-20">
									<div class="form-group">
													...
													<select multiple data-role="tagsinput" name="document_tags[]" id="document_tags">
                                                                                                            @if(!empty($po->document_tags))
                                                                                                                @foreach($po->document_tags as $tag)
									...
									<div class="form-group">
										...
										<textarea class="form-control" name="memo_order" id="memo_order" rows="5">{{ $po->memo_order }}</textarea>
									</div>
								</div>
								<div class="col-xs-4">
									<div class="row">
										<div class="col-md-12 text-right p-t-10">
											...
											<p class="form-control-static inline-p">
												<span class="sub-total">{{ number_format((float)$po->product_subtotal , 2, '.', '') }}</span>
											</p>
										</div>
									...
									<div class="row">
										...
										<div class="form-group col-md-4">
											<input type="number" min="0" value="{{ $po->discount }}" name="discount_value"
												class="form-control discount_value">
										</div>
                                                                                <div class="form-group col-md-4" >
                                                                                    <select class="form-control" name="discount_type" id="discount_type">
                                                                                        <option
                                                                                                @if($po->discount_type == 'percent') selected @endif value="percent">%</option>
                                                                                        <option
                                                                                                @if($po->discount_type == 'fixed') selected @endif value="fixed">@lang('modules.invoices.amount')</option>
                                                                                    </select>
                                                                                </div>
									</div>
										...
										<div class="form-group col-md-4">
											<input type="number" min="0" value="{{ $po->freight_value }}" name="freight_value"
												class="form-control freight_value">
										</div>
                                                                                <div class="form-group col-md-4" >
                                                                                    <select class="form-control" name="freight_type" id="freight_type">
                                                                                        <option
                                                                                                @if($po->freight_type == 'percent') selected @endif value="percent">%</option>
                                                                                        <option
                                                                                                @if($po->freight_type == 'fixed') selected @endif value="fixed">@lang('modules.invoices.amount')</option>
                                                                                    </select>
                                                                                </div>
									</div>
									...
									<div class="row m-t-5 font-bold">
										<div class="col-md-12 text-right p-t-10">
											...
											<p class="form-control-static inline-p">
												<span class="total">{{ number_format((float)$po->total_amount, 2, '.', '') }}</span>
											</p>
										</div>


										<input type="hidden" class="total-field" name="total_amount" value="{{ round($po->total_amount, 2) }}">
									</div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="fileinput fileinput-new input-group" data-provides="fileinput">
                                            <div class="form-control" data-trigger="fileinput">
                                                ...
                                                <span class="fileinput-filename">{{ $po->specification_file }}</span>
                                            </div>
                                        ...
                                        @if(!is_null($po->specification_file))
                                            <a target="_blank"
                                               href="{{ asset_url('purchase-orders/'.$po->specification_file) }}">View File</a>
                                        @endif
                                    </div>
...
<script>
    ...
    $(function() {
         ...
         $('.product-vendor-items-'+{{$po->vendor_id}}).show();
    ...
    function getVendorDetail(par){
            ...
            var url = "{{ route('member.purchase-orders.get-vendor-detail',':id') }}";
        ...
        $.easyAjax({
            url: '{{route('member.purchase-orders.update', $po->id)}}',
        ...
        $('#updatePayments').on('keyup change', '.quantity,.cost_per_item,.item_name, .discount_value, .freight_value', function () {
        ...
        $('#updatePayments').on('change', '.type, #discount_type, #freight_type', function () {
            var quantity = $(this).closest('.item-row').find('.quantity').val();
...
<script>
        ...
        $('#sel_product').on('click', function (event) {
            ...
            $.each($("input[name='select_product']:checked"), function(){
                ...
                $.easyAjax({
                    url: '{{ route('member.purchase-orders.update-item') }}',
                    type: "GET",
