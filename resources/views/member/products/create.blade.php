...
@section('content')
    ...
    <div class="row wa-new-changes m-t-20">
        <div class="col-md-12">
                        ...
                        {!! Form::open(['id'=>'tab-item','class'=>'ajax-form','method'=>'POST', 'files' => true]) !!}
                            <div class="panel-tab">
                                ...
                                <div class="panel-tab-body">
                                    <div class="panel-tab-item active" tab="item">@include('member.products.tabs.item')</div>
                                    
                                    <div class="panel-tab-item" tab="pricing">@include('member.products.tabs.pricing')</div>
                                    <div class="panel-tab-item" tab="Additional Product Information">@include('member.products.tabs.specification')</div>
                                    <div style="display: none;" class="panel-tab-item" tab="workroom">@include('member.products.tabs.workroom')</div>
                                </div>
                            </div>
                            <div class="form-actions text-right">
                                ...
                                <a href="{{route('member.products.index')}}" class="btn btn-default">@lang('app.cancel')</a>
                            </div>
                          {!! Form::close() !!}
                    </div>
                </div>
    ...
    @if($product->id != 0)
    @include('member.products.tabs.purchase-order')
    @endif
...
@push('footer-script')
    ...
    <script>
        ...
        const saveForm = (tabId) => {
            ...
            var urlJS = "{{$product->id == 0 ? route('member.products.store') : route('member.products.update', [$product->id])}}";
            var typeJS = "{{$product->id == 0 ? 'POST' : 'PUT'}}";
            
            // for copy
            if(copy == 1) {
                urlJS = "{{route('member.products.store')}}";
                typeJS = "{{'POST'}}";
                redirectJS = 'true';
            }
    ...
    @include('member.products.tabs.picture-script')
    @include('member.products.tabs.pricing-script')
@endpush
