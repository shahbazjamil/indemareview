            ...
            <div class="panel panel-inverse">
                ...
                <div class="panel-wrapper collapse in" aria-expanded="true">
                    <div class="panel-body p-0 p-t-20">
                        {!! Form::open(['id'=>'convertEstimate','class'=>'ajax-form','method'=>'POST','enctype' => 'multipart/form-data']) !!}
                        <div class="form-body">
...
{{--														</p>--}}
                                            ...
                                            <div class="col-md-1">
												<div class="hd">@lang('modules.invoices.type')</div>
...
<script>
    ...
    $('#save-form').click(function () {
        ...
        let form = $('#convertEstimate');
        $.ajax({
            url: '{{route('member.all-invoices.store')}}',
            container: '#convertEstimate',
            type: "POST",
            redirect: true,
            ...
            //  data: $('#convertEstimate').serialize()
        });
    });
    ...
    $('#convertEstimate').on('click', '.remove-item', function () {
    ...
    $('#convertEstimate').on('keyup change', '.markup', function () {
            var quantity = 0;
     ...
     $('#convertEstimate').on('keyup change', '.markup_fix', function () {
            var quantity = 0;
            if($(this).closest('.item-row').find('.quantity').val()) {
                 quantity = $(this).closest('.item-row').find('.quantity').val();
            }
    ...
    $('#convertEstimate').on('keyup change', '.quantity,.cost_per_item,.item_name, .discount_value, .card_processing_value, .deposit_request,  .tax_on_total , .sale_price, .shipping_price', function () {
            var quantity = 0;
            if($(this).closest('.item-row').find('.quantity').val()) {
                 quantity = $(this).closest('.item-row').find('.quantity').val();
            }
    ...
    $('#convertEstimate').on('change','.type, #discount_type , #card_processing_type, #deposit_request_type', function () {
        var quantity = 0;
            if($(this).closest('.item-row').find('.quantity').val()) {
                 quantity = $(this).closest('.item-row').find('.quantity').val();
            }
