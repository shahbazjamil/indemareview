{{--@component('mail::automation_message')--}}
{{--    --}}{{-- Header --}}
{{--    @component('mail::header', ['url' => isset($company) ? $company['website'] : config('app.url') ])--}}
{{--        <img src="{{ isset($company) ? $company['logo_url'] : config('app.logo') }}" style="max-height: 45px; background-image: url('.{{ isset($company) ? $company['login_background_url'] : '' }}.')" alt="">--}}
{{--    @endcomponent--}}
...
<body>
@if(!empty($audit_id))
    <img border=0 width=1 alt="" height=1 src="{{ route('audit.update.email.status',[$audit_id]) }}" />
@endif
