                    ...
                    <div class="row">
                                    ...
                                    @foreach($invoice->items as $item)
                                        @if($item->type == 'item')
                                            <tr>
                                                ...
                                                <td class="text-right"> {!! currency_position($item->shipping_price, $invoice->currency->currency_symbol) !!} </td>
                        ...
                        <div class="col-md-12">
                            <div class="pull-right m-t-30 text-right">
                                ...
                                <p>
                                    Deposit Request : {{ currency_position($invoice->deposit_req, $invoice->currency->currency_symbol) }}
                                </p>
...
<script>
    ...
    @if($credentials->stripe_status == 'active')
        ...
        var handler = StripeCheckout.configure({
            ...
            token: function(token) {
                ...
                $.easyAjax({
                    ...
                    type: "POST",
                    redirect: true,
                    data: {token: token, "_token" : "{{ csrf_token() }}", name: name, line1: line1, postal_code: postal_code, city: city, state: state, country: country, is_deposit : is_deposit}
                })
            }
        });
        ...
        $('#stripeAddress').submit(function(e){
            ...
            handler.open({
                name: '{{ $companyName }}',
                amount: {{ $invoice->deposit_req*100 }},
    ...
    function showButton(type){
     ...
     function offlinePayment() {
        ...
        $.ajaxModal('#package-offline', '{{ route('client.invoices.offline-payment')}}?offlineId='+offlineId+'&invoiceId='+'{{$invoice->id}}&is_deposit=1');
    ...
    @if($credentials->razorpay_status == 'active')
        $('#razorpayPaymentButton').click(function() {
            console.log('{{ $invoice->currency->currency_code }}');
                var amount = {{ $invoice->deposit_req*100 }};
            ...
            function confirmRazorpayPayment(id,invoiceId,rData) {
                $.easyAjax({
                    ...
                    data: {paymentId: id,invoiceId: invoiceId,rData: rData,_token:'{{csrf_token()}}', is_deposit : '1'}
                })
            }
