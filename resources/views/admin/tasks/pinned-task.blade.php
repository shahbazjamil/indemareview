            ...
            <table class="table" id="pinnedTaskTable">
                ...
                @forelse($pinnedItems as $key=>$pinnedItem)
                    <tr id="project-{{ $pinnedItem->id }}">
                        ...
                        <td><a href="javascript:;" data-task-idd="{{ $pinnedItem->id }}" class="show-task-details btn btn-info btn-circle" data-toggle="tooltip" data-original-title="@lang('app.view')"><i class="fa fa-search" aria-hidden="true"></i> </a></td>
                    </tr>
                @empty
                    <tr>
...
<script>
    ...
    $('#pinnedTaskTable').on('click', '.show-task-details', function () {
        $('#taskCategoryModal').modal('hide');

        $(".right-sidebar").slideDown(50).addClass("shw-rside");

        var id = $(this).data('task-idd');
        var url = "{{ route('admin.all-tasks.show',':id') }}";
        url = url.replace(':id', id);
