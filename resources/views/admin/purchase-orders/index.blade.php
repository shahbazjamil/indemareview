...
<div class="row bg-title">
    ...
    <div @if($totalRecords == 0) style="display: none;"  @endif class="border-bottom col-xs-12 p-t-10 p-b-10">
        ...
        <a href="{{ route('admin.purchase-orders.archive') }}"  class="btn btn-outline btn-danger btn-sm">@lang('app.menu.viewArchive') <i class="fa fa-trash" aria-hidden="true"></i></a>
        <a href="{{ route('admin.purchase-orders.create') }}" class="btn btn-outline btn-success btn-sm">+ Create Purchase Order </a>
...
<div @if($totalRecords > 0) style="display: none;"  @endif class="row flex-row flex-wrap nolist-content flex-align-start">
		...
		<div class="col-md-8">
			...
			<a href="{{ route('admin.purchase-orders.create') }}" class="btn-black">+ Create Purchase Order </a>
        ...
        $('body').on('click', '.archive', function(){
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    var url = "{{ route('admin.purchase-orders.archive-delete',':id') }}";
        ...
        $('body').on('click', '.sa-params', function(){
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    var url = "{{ route('admin.purchase-orders.destroy',':id') }}";
                    url = url.replace(':id', id);
                    var token = "{{ csrf_token() }}";
                    $.easyAjax({
                        type: 'POST',
