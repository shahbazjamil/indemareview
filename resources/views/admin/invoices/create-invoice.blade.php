    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="panel panel-inverse">
                <div class="panel-heading p-b-10 m-b-20"> @lang('app.create') @lang('app.timeLog') @lang('app.invoice')</div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div>
                                            <div class="input-group">
                                                ...
                                                <input type="text"  class="form-control readonly-background"  name="invoice_number" id="invoice_number" value="@if(is_null($lastInvoice))1 @else{{ ($lastInvoice) }}@endif">
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <div class="col-md-12">
                                                    <select class="select2 form-control" onchange="getCompanyName()" data-placeholder="Choose Project" name="project_id" id="project_id">
                                                        @foreach($projects as $project)
                                                            <option @if($sel_project_id == $project->id) selected @endif value="{{ $project->id }}">{{ ucwords($project->project_name) }}</option>
                                                        @endforeach
                                                    </select>
...
<script>
    ...
    $('#save-form').click(function(){
        ...
        $.easyAjax({
            url:'{{route('admin.client-invoice.store')}}',
    ...
    $('#storePayments').on('click','.remove-item', function () {
        $(this).closest('.item-row').fadeOut(300, function() {
            ...
            $('select.customSequence').each(function(index){
             ...
             $('select.customSequenceGroup').each(function(index){
                $(this).attr('name', 'groups['+index+']');
                $(this).attr('id', 'groupselect'+index+'');
            });
        ...
        $.easyAjax({
            ...
            success: function(response) {
                ...
                $('#sortable').find(".quantity").each(function (index, element) {
                    ...
                    var itemRow2 = $(this).closest('.item-row').find('select.type2');
